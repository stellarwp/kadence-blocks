...
#   3. Compile pending changelog entries into readme.txt
#
...
#
# Usage:
#   dev_scripts/release-prep.sh <version> [--date <date>] [--dry-run]
#   composer run release:prep 3.8.0
#
set -euo pipefail
...
if [ -z "$VERSION" ]; then
  ...
  echo "Usage: dev_scripts/release-prep.sh <version> [--date <date>] [--dry-run]" >&2
  exit 1
fi
...
dev_scripts/write-changelog.sh "$VERSION" "$@"
