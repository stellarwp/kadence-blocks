...
class Editor_Assets {
	...
	public function __construct() {
		add_action( 'init', array( $this, 'on_init_editor_assets' ), 10 );
		add_action( 'enqueue_block_editor_assets', array( $this, 'editor_assets_variables' ) );
		add_action( 'enqueue_block_editor_assets', array( $this, 'editor_plugin_enqueue' ) );
		add_action( 'current_screen', array( $this, 'early_editor_assets' ) );
			...
			if ( is_array( $heading_font ) ) {
				...
				if ( isset( $heading_font['google'] ) && $heading_font['google'] ) {
					...
					if ( is_array( $variants ) ) {
						if ( $is_body ) {
							$new_weight_variant = array( $body_font['variant'] );
						} else {
	...
	public function get_all_google_fonts() {
		if ( is_null( self::$google_fonts ) ) {
			self::$google_fonts = file_exists( KADENCE_BLOCKS_PATH . 'includes/gfonts-array.php' ) ? include KADENCE_BLOCKS_PATH . 'includes/gfonts-array.php' : array();
		}
		...
		if ( function_exists( 'Kadence\kadence' ) ) {
			...
			if ( ! empty( $base_font['family'] ) ) {
				if ( isset( $base_font['google'] ) && $base_font['google'] ) {
					...
					if ( is_array( $variants ) ) {
						$new_weight_variant = array( $base_font['variant'] );
						$heading_font = \Kadence\kadence()->option( 'heading_font' );
						foreach ( array( 'h1_font', 'h2_font', 'h3_font', 'h4_font', 'h5_font', 'h6_font' ) as $option ) {
							$h_font = \Kadence\kadence()->option( $option );
				...
				} else if ( isset( $base_font['family'] ) && ! empty( $base_font['family'] ) && substr( $base_font['family'], 0, strlen( '-apple-system' ) ) === '-apple-system' ) {
					// System Font.
					$weights = array(
						array( 'value' => '', 'label' => __( 'Inherit', 'kadence-blocks' ) ),
						array( 'value' => '100', 'label' => __( 'Thin 100', 'kadence-blocks' ) ),
		...
		if ( function_exists( 'Kadence\kadence' ) ) {
			...
			if ( ! empty( $button_font['family'] ) ) {
				...
				if ( isset( $button_font['google'] ) && $button_font['google'] ) {
					...
					if ( is_array( $variants ) ) {
						$new_weight_variant = array( $button_font['variant'] );
						$heading_font = \Kadence\kadence()->option( 'heading_font' );
						foreach ( array( 'h1_font', 'h2_font', 'h3_font', 'h4_font', 'h5_font', 'h6_font' ) as $option ) {
							$h_font = \Kadence\kadence()->option( $option );
				...
				} else if ( isset( $button_font['family'] ) && ! empty( $button_font['family'] ) && substr( $button_font['family'], 0, strlen( '-apple-system' ) ) === '-apple-system' ) {
					// System Font.
					$weights = array(
						array( 'value' => '', 'label' => __( 'Inherit', 'kadence-blocks' ) ),
						array( 'value' => '100', 'label' => __( 'Thin 100', 'kadence-blocks' ) ),
