...
class Kadence_Blocks_Progress_Bar_Block extends Kadence_Blocks_Abstract_Block {
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		...
		$content .= '<div id="kb-progress-bar' . $unique_id . '" class="kb-progress-bar" role="progressbar" aria-label="' . esc_attr($attributes['label']) . '" aria-valuemin="' . esc_attr( $progress_min ) . '" aria-valuemax="' . ( $is_relative ? 100 : $progress_max ) . '">' . ( $this->get_label( $attributes, 'inside' ) ) . '</div>';
