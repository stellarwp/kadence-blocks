...
class Kadence_Blocks_Progress_Bar_Block extends Kadence_Blocks_Abstract_Block {
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		...
		$content .= '<div id="kb-progress-bar' . $unique_id . '" class="kb-progress-bar" role="progressbar" aria-label="' . ( empty($attributes['label']) ? __('Progress', 'kadence-blocks') : esc_attr( strip_tags( $attributes['label'] ) ) ) . '" aria-valuemin="' . esc_attr( $progress_min ) . '" aria-valuemax="' . ( $is_relative ? 100 : $progress_max ) . '">' . ( $this->get_label( $attributes, 'inside' ) ) . '</div>';
	...
	private function get_label( $attributes, $position ) {
		...
		$label = '<span role="textbox" class="kt-progress-label" contenteditable="false" aria-label="' . esc_attr__( 'Progressbar Label', 'kadence-blocks' ). '">' . $attributes['label'] . '</span>';
