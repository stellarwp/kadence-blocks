...
class Kadence_Blocks_Singlebtn_Block extends Kadence_Blocks_Abstract_Block {
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		$button_attributes = $this->get_button_attributes( $unique_id, $attributes );

		$css->set_style_id( 'kb-' . $this->block_name . $unique_style_id );

		$sizes = array( 'Desktop', 'Tablet', 'Mobile' );
		foreach ( $sizes as $size ) {
			$this->sized_dynamic_styles( $css, $button_attributes, $unique_id, $size );
		}
	 ...
	 * @return array
	 */
	private function get_button_attributes( $unique_id, $attributes ) {
		global $wp_meta_keys;
	...
	private function merge_defaults( $attributes ) {
		$registry = WP_Block_Type_Registry::get_instance()->get_registered( 'kadence/singlebtn' );
