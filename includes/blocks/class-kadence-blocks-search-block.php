...
class Kadence_Blocks_Search_Block extends Kadence_Blocks_Abstract_Block {
	...
	public function build_css( $attributes, $css, $unique_id, $unique_style_id ) {
		...
		$css->set_selector( '.wp-block-kadence-search .kadence-search-button' . $unique_id . ' .kb-button' );
		if ( 'fixed' === $width_type ) {
		...
		// Icon.
		$css->set_selector( '.wp-block-kadence-search .kadence-search-button' . $unique_id . ' .kb-button .kb-svg-icon-wrap' );
		...
		// Icon Hover.
		$css->set_selector( '.wp-block-kadence-search .kadence-search-button' . $unique_id . ' .kb-button:hover .kb-svg-icon-wrap' );
		if ( ! empty( $attributes['iconColorHover'] ) ) {
			$css->add_property( 'color', $css->render_color( $attributes['iconColorHover'] ) );
		}
		// Hover.
		$css->set_selector( '.wp-block-kadence-search .kadence-search-button .kb-btn' . $class_id . '.kt-button:hover' );
		...
		if ( isset( $attributes['displayHoverShadow'] ) && true === $attributes['displayHoverShadow'] ) {
			if ( ( 'gradient' === $bg_type || 'gradient' === $bg_hover_type ) && isset( $attributes['shadowHover'][0]['inset'] ) && true === $attributes['shadowHover'][0]['inset'] ) {
				...
				$css->set_selector( '.wp-block-kadence-search .kadence-search-button' . $unique_id . ' .kb-button:hover::before' );
			}
		...
		if ( 'gradient' === $bg_type && 'normal' === $bg_hover_type && ! empty( $attributes['backgroundHover'] ) ) {
			$css->set_selector( '.wp-block-kadence-search .kadence-search-button' . $unique_id . ' .kb-button:hover::before' );
			$css->add_property( 'background', $css->render_color( $attributes['backgroundHover'] ) );
			$css->set_selector( '.wp-block-kadence-search .kadence-search-button' . $unique_id . ' .kb-button::before' );
		...
		if ( isset( $attributes['onlyIcon'][0] ) && '' !== $attributes['onlyIcon'][0] && true == $attributes['onlyIcon'][0] ) {
			$css->set_selector( '.wp-block-kadence-search .kadence-search-button' . $unique_id . ' .kb-button .kt-btn-inner-text' );
	...
	public function build_html( $attributes, $unique_id, $content, $block_instance ) {
		$outer_classes = array( 'wp-block-kadence-search', 'wp-block-kadence-search' . $unique_id );
		$wrapper_args = array(
	...
	private function build_button( $attributes, $unique_id, $content ) {
		$class_id = $unique_id;
		$outer_classes = array( 'kadence-search-button', 'kadence-search-button' . $unique_id );
		$wrapper_args = array(
			'class' => implode( ' ', $outer_classes ),
		...
		$classes[] = ! empty( $attributes['icon'] ) || !isset( $attributes['icon'] ) ? 'kt-btn-has-svg-true' : 'kt-btn-has-svg-false';
		...
		$svg_icon   = '';
		if ( ! empty( $attributes['icon'] ) || !isset( $attributes['icon'] ) ) {
			$icon = !empty( $attributes['icon'] ) ? $attributes['icon'] : 'fe_search';
			...
			$svg_icon = Kadence_Blocks_Svg_Render::render( $icon, $fill, $stroke_width );
		}
		$icon_left  = ! empty( $svg_icon ) && ! empty( $attributes['iconSide'] ) && 'left' === $attributes['iconSide'] ? '<span class="kb-svg-icon-wrap kb-svg-icon-' . esc_attr( $icon ) . ' kt-btn-icon-side-left">' . $svg_icon . '</span>' : '';
		$icon_right = ! empty( $svg_icon ) && ! empty( $attributes['iconSide'] ) && 'right' === $attributes['iconSide'] ? '<span class="kb-svg-icon-wrap kb-svg-icon-' . esc_attr( $icon ) . ' kt-btn-icon-side-right">' . $svg_icon . '</span>' : '';
		$html_tag   = 'button';
