	...
	public static function check_download_request() {
		if ( ! empty( $_GET['kadence-form-download'] ) ) {
			...
			if ( isset( $upload_dir['basedir'] ) ) {
				...
				$form_ajax_actions = new KB_Ajax_Advanced_Form();
				$form_upload_root = $form_ajax_actions->get_kadence_form_root_upload_dir();

				$paths_match = realpath( $path ) === $path;
				$path_contains_upload_root = strpos(realpath( $path ), $form_upload_root) !== false;

				if( $paths_match && $path_contains_upload_root && file_exists( $upload_dir['basedir'] . '/' . $_GET['kadence-form-download'] ) ) {
