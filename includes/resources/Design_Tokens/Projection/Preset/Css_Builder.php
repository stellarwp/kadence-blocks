...
/**
 * Builds the scoped CSS for selectable Kadence block presets for the single active token set.
 *
 ...
 *      var(--kb-token--<target>) so the preset chains to the active set's canonical token, and a literal
...
final class Css_Builder {
	...
	/**
	 * Per-request memo of the collected preset structure, keyed on the set slug AND its store version, so the
	...
	/**
	 * Build the active set's preset CSS: the canonical preset-var definitions plus the scoped retarget
	...
	/**
	 * The active set's canonical `--kb-token--preset--*` declarations, without a selector wrapper — the raw
	...
	/**
	 * Build (uncached) the active set's preset CSS: the canonical preset-var definitions followed by the
	 ...
	 * aliased preset chains to the set's canonical token. Memoized per slug for the request.
	 *
	 * @since TBD
	 *
	...
	/**
	 * Emit the canonical `--kb-token--preset--*` definitions from the active set's collected presets. The
	 * value preserves alias indirection canonically, so the preset chains to the active set's token.
	 *
	 * @since TBD
	 *
	...
	/**
	 * Emit the per-preset scoped rules from the active set's collected presets: per (block, preset) a
