 ...
 *
 * core/button reuses the Kadence button's exact variant mechanism: the shared Variant\Css_Builder retargets
 * --global-palette-btn-* per selected variant, and these rules make the native button link read those slots
...
final class Button implements Styles {
	...
	public function css( bool $owns_default ): string {
		...
		return
			...
			$fill . ':not(.has-background){background-color:var(--global-palette-btn-bg);}'
			. $fill . ':not(.has-text-color){color:var(--global-palette-btn);}'
			. $fill . ':not(.has-background):hover,' . $fill . ':not(.has-background):focus{background-color:var(--global-palette-btn-bg-hover);}'
			. $fill . ':not(.has-text-color):hover,' . $fill . ':not(.has-text-color):focus{color:var(--global-palette-btn-hover);}'
			// Outline: no fill, so the background slot becomes the border + text color.
			. $outline . ':not(.has-text-color){color:var(--global-palette-btn-bg);}'
			. $outline . ':not(.has-border-color){border-color:var(--global-palette-btn-bg);}'
			. $outline . ':not(.has-text-color):hover,' . $outline . ':not(.has-text-color):focus{color:var(--global-palette-btn-bg-hover);}'
			. $outline . ':not(.has-border-color):hover,' . $outline . ':not(.has-border-color):focus{border-color:var(--global-palette-btn-bg-hover);}';
	}
}
