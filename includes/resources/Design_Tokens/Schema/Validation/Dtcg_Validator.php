 ...
 * as far as this ticket's scope: preset `tokens` map values must be alias-or-literal; richer
...
final class Dtcg_Validator {
	...
	/**
	 * Validate the $extensions layer to this scope: every preset `tokens` map value must be an
	...
	/**
	 * A preset token value must be an alias or a non-empty literal scalar. The target token's
	...
	private function validate_extension_value( $value, string $path ): ?Validation_Error {
		...
		return new Validation_Error(
			$path,
			Validation_Error::get_code_value_invalid(),
			'A preset token value must be an alias or a non-empty literal.'
		);
	}
