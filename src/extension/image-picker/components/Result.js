...
export default function Result(props) {
	...
	const downloadLabel = downloadError
		? __('Download Error', 'kadence-blocks')
		: __('Download Image', 'kadence-blocks');
	return (
		<article className={itemClass}>
			<div className="result-wrap">
				<Button ref={resultButton} className="image-wrap" label={__('Select Image')} onClick={handleSelection}>
					<img src={imageSrc} alt={alt} className={'img'} />
					<div class="image-select-container">
						<div className={'kb-image-selection-button'}>
							{isSelected ? <Icon icon={CheckIcon}></Icon> : ''}
						</div>
					</div>
				</Button>
