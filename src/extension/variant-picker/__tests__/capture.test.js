...
// its `VariantPicker` component. `capturedTokens` never renders it, so stub the module out.
...
function seedCatalog() {
	window.kadenceDesignTokensVariants = {
		active: 'default',
		sets: {
			default: {
				[BLOCK]: {
					default: 'primary',
					variants: [{ slug: 'primary', label: 'Primary' }],
					properties: [
...
describe('capturedTokens', () => {
	...
	it('captures an edited color as its normalized literal', () => {
		const attributes = { kbVariant: 'primary', background: '#FF0000' };
	...
	it('falls back to the preset value when a control is not edited', () => {
		const attributes = { kbVariant: 'primary' };
	...
	it('uses the default preset when no variant is selected', () => {
	...
	it('layers an edit over the preset for the rest of the surface', () => {
		const attributes = { kbVariant: 'primary', background: '#00FF00' };
