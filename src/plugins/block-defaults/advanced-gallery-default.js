...
class KadenceAdvancedGalleryDefault extends Component {
	...
	render() {
		...
		const headingConfig = ( configuration && configuration[ 'kadence/advancedgallery' ] ? configuration[ 'kadence/advancedgallery' ] : {} );
		const marginTypes = [
			{ key: 'px', name: __( 'px' ) },
			{ key: 'em', name: __( 'em' ) },
			{ key: '%', name: __( '%' ) },
			...
			<Fragment>
				...
				{ isOpen ?
					<Modal
						...
						onRequestClose={ () => {
							this.saveConfig( 'kadence/advancedgallery', headingConfig );
						} }>
						<PanelBody
