...
function KadenceButtonDefault(props) {
    ...
    useEffect(() => {
        ...
        if (!configuration['kadence/advancedbtn']) {
    ...
    const defineWidthType = (type) => {
        if ('full' === type) {
            saveConfigState('forceFullwidth', true);
            saveConfigState('widthType', type);
        } else {
            saveConfigState('forceFullwidth', false);
            saveConfigState('widthType', type);
        }
    };
    const defineWidthTypeToggle = (value) => {
        if (value) {
            saveConfigState('forceFullwidth', true)
            saveConfigState('widthType', 'full');
        } else {
            saveConfigState('forceFullwidth', false)
            saveConfigState('widthType', 'full');
        }
        ...
        <Fragment>
            ...
            {isOpen ?
                <Modal
                    ...
                    onRequestClose={() => {
                        saveConfig('kadence/advancedbtn', buttonConfig);
                    }}>
                    <PanelBody>
                        <SelectControl
                    ...
                    <div className="kb-modal-footer">
                        {!resetConfirm && (
                            <Button className="kt-defaults-save" isDestructive
                                    disabled={(JSON.stringify(configuration['kadence/advancedbtn']) === JSON.stringify({}) ? true : false)}
                                    onClick={() => {
