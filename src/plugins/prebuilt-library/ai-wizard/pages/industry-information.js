...
export function IndustryInformation() {
	...
	function getLocationPlaceholderText() {
		if (locationType !== LOCATION_ONLINE_ONLY) {
			const currentLocation = LOCATION_TYPES.filter(
				(location) => location.value === locationType
			);

			return currentLocation.length && currentLocation[0]?.placeholder
				? currentLocation[0].placeholder
				: "...";
		}
	...
	function getLocationHelpText() {
		if (locationType !== LOCATION_ONLINE_ONLY) {
			...
			return currentLocation.length && currentLocation[0]?.help
				? currentLocation[0].help
				: "";
		}
