...
function CloudSections ( {
	...
	async function onInsertContent( pattern ) {
		...
		if ( action?.[0]?.url ) {
			const response = await getPattern( tab, 'pattern', ( pattern?.id ? pattern.id : '' ), 'light', ( action?.[0]?.url ? action[0].url : '' ), action?.[0]?.key ? action[0].key : '' );
			if ( response ) {
				try {
	...
	async function getLibraryContent( tempReload ) {
		...
		const response = await getPatterns( tab, tempReload, action?.[0]?.url ? action[0].url : '', action?.[0]?.key ? action[0].key : '' );
		if ( response === 'failed' ) {
...
// 	} ) ),
// )( CloudSections );
const CloudLibraryWrapper = withDispatch(
