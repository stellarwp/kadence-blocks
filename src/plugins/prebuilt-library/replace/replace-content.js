...
export default function replaceContent( content, aiContent, categories, context, variation, isHTML = false ) {
	...
	switch ( currentCategory ) {
				...
				for ( let index = 0; index < columnsContent?.columns.length; index++ ) {
					...
					if ( columnsContent?.columns?.[ index ]?.[ 'title-medium' ] ) {
						content = content.replace(
							...
							columnsContent?.columns?.[ index ]?.[ 'title-medium' ]
						);
					}
					// Paragraph.
					if ( columnsContent?.columns?.[ index ]?.[ 'sentence-short' ] ) {
						content = content.replace(
							...
							columnsContent?.columns?.[ index ]?.[ 'sentence-short' ]
						);
					}
				}
			}
		...
		case 'text':
			...
			if ( baseContent?.heading?.medium ) {
				content = content.replace(
					/Briefly and concisely explain what you do for your audience./g,
					baseContent?.heading?.medium
				);
			}
			// Paragraph long
			...
			if ( baseContent?.sentence?.short ) {
				content = content.replace(
					/Consider using this if you need to provide more context on why you do what you do. /g,
					baseContent?.sentence?.short
				);
				content = content.replace(
					/Consider using this if you need to provide more context on why you do what you do. Be engaging./g,
					baseContent?.sentence?.short
				);
			}
			// overline
			...
			if ( columnsContent?.columns ) {
				for ( let index = 0; index < columnsContent?.columns.length; index++ ) {
					...
					if ( columnsContent?.columns?.[ index ]?.[ 'title-short' ] ) {
						content = content.replace(
							'Add a short title',
							columnsContent?.columns?.[ index ]?.[ 'title-short' ]
						);
					}
					// Paragraph.
					if ( columnsContent?.columns?.[ index ]?.[ 'sentence-short' ] ) {
					...
					if ( columnsContent?.columns?.[ index ]?.[ 'sentence-medium' ] ) {
						content = content.replace(
							...
							columnsContent?.columns?.[ index ]?.[ 'sentence-medium' ]
						);
					}
				}
			}
			...
			if ( heroContent?.cards ) {
				for ( let index = 0; index < heroContent?.cards.length; index++ ) {
					...
					if ( heroContent?.cards?.[ index ]?.[ 'sentence-short' ] ) {
						content = content.replace(
							'Use this space to add a short description.',
							heroContent?.cards?.[ index ]?.[ 'sentence-short' ]
						);
					}
					// Paragraph medium.
					if ( heroContent?.cards?.[ index ]?.[ 'sentence-medium' ] ) {
						content = content.replace(
							...
							heroContent?.cards?.[ index ]?.[ 'sentence-medium' ]
						);
					}
					// Button.
		...
		case 'image':
			...
			if ( baseContent?.heading?.medium ) {
				content = content.replace(
					/Add a short, consistent heading for your image./g,
					baseContent?.heading?.medium
				);
			}
			// Short Headline.
			...
			if ( baseContent?.sentence?.short ) {
				content = content.replace(
					/Use this paragraph to add supporting context. Consider your audience and what matters to them, and provide insights that support your topic./g,
					baseContent?.sentence?.short
				);
			}
			// Button
		...
		case 'title-or-header':
			...
			if ( titleContent?.heading?.medium ) {
				content = content.replace(
					/Craft a captivating title for this section to attract your audience./g,
					titleContent?.heading?.medium
				);
				content = content.replace(
					/Craft a captivating title for the upcoming section to attract your audience./g,
					titleContent?.heading?.medium
				);
			} else if ( baseContent?.heading?.medium ) {
				content = content.replace(
		...
		case 'media-text':
			...
			if ( baseContent?.heading?.medium ) {
				content = content.replace(
					/Add a compelling title for your section to engage your audience./g,
					baseContent?.heading?.medium
				);
			}
			// Paragraph
			if ( baseContent?.sentence?.short ) {
				content = content.replace(
					/Use this paragraph section to get your website visitors to know you. Consider writing about you or your organization, the products or services you offer, or why you exist. Keep a consistent communication style./g,
					baseContent?.sentence?.short
				);
			}
			// overline
					...
					if ( listContent?.list?.[ index ]?.[ 'list-item-short' ] ) {
						if ( ! isHTML ) {
							content = content.replace(
								...
								`"text":"${ listContent?.list?.[ index ]?.[ 'list-item-short' ] }"`
							);
						}
						content = content.replace(
							'Add a list item',
							listContent?.list?.[ index ]?.[ 'list-item-short' ]
						);
					}
				}
				for ( let index = 0; index < listContent?.list.length; index++ ) {
		...
		case 'accordion':
			...
			if ( accordionContent?.heading?.medium ) {
				content = content.replace(
					/A brief headline here will add context for the section/g,
					accordionContent?.heading?.medium
				);
			} else if ( baseContent?.heading?.medium ) {
				content = content.replace(
					/A brief headline here will add context for the section/g,
			...
			if ( accordionContent?.sentence?.medium ) {
				content = content.replace(
					/Use this space to provide your website visitors with a brief description on what to expect before clicking on a section title./g,
					accordionContent?.sentence?.medium
				);
			...
			if ( accordionContent?.accordion ) {
				for ( let index = 0; index < accordionContent?.accordion.length; index++ ) {
					...
					if ( accordionContent?.accordion?.[ index ]?.[ 'title-medium' ] ) {
						if ( ! isHTML ) {
							content = content.replace(
								`"text":"Add a section title that is relevant for your readers."`,
								`"text":"${ accordionContent?.accordion?.[ index ]?.[ 'title-medium' ] }"`
							);
						}
					}
				}
				for ( let index = 0; index < accordionContent?.accordion.length; index++ ) {
					...
					if ( accordionContent?.accordion?.[ index ]?.[ 'title-medium' ] ) {
						content = content.replace(
							'Add a section title that is relevant for your readers.',
							accordionContent?.accordion?.[ index ]?.[ 'title-medium' ]
						);
						content = content.replace(
							'tab-addasectiontitlethatisrelevantforyourreaders',
							`tab-${ stripStringRender(
								accordionContent?.accordion?.[ index ]?.[ 'title-medium' ].toString()
							) }`
						);
						content = content.replace(
							'tab-addasectiontitlethatisrelevantforyourreaders',
					...
					if ( accordionContent?.accordion?.[ index ]?.[ 'paragraph-medium' ] ) {
						content = content.replace(
							...
							accordionContent?.accordion?.[ index ]?.[ 'paragraph-medium' ]
						);
					}
				}
			}
		...
		case 'tabs':
			...
			if ( baseContent?.sentence?.medium ) {
				content = content.replace(
					/Tabs are a helpful way that allow users to view a group of related data one at a time. Add a brief description of what your tabbed section is about./g,
					baseContent?.sentence?.medium
				);
			}
			// overline
			...
			if ( tabsContent?.tabs ) {
				for ( let index = 0; index < tabsContent?.tabs.length; index++ ) {
					...
					if ( tabsContent?.tabs?.[ index ]?.[ 'title-short' ] ) {
						if ( ! isHTML ) {
							content = content.replace(
								`"text":"Tab name"`,
								`"text":"${ tabsContent?.tabs?.[ index ]?.[ 'title-short' ] }"`
							);
						}
					}
					if ( tabsContent?.tabs?.[ index ]?.[ 'title-medium' ] ) {
						if ( ! isHTML ) {
							content = content.replace(
								`"text":"Give this tab a concise name"`,
								`"text":"${ tabsContent?.tabs?.[ index ]?.[ 'title-medium' ] }"`
							);
						}
					}
				}
				for ( let index = 0; index < tabsContent?.tabs.length; index++ ) {
					...
					if ( tabsContent?.tabs?.[ index ]?.[ 'title-short' ] ) {
						...
						content = content.replace(
							'tab-tabname',
							`tab-${ stripStringRender( tabsContent?.tabs?.[ index ]?.[ 'title-short' ].toString() ) }`
						);
						content = content.replace(
							'tab-tabname',
							`tab-${ stripStringRender( tabsContent?.tabs?.[ index ]?.[ 'title-short' ].toString() ) }`
						...
						content = content.replace(
							'Add a brief title',
							tabsContent?.tabs?.[ index ]?.[ 'title-short' ]
						);
					...
					if ( tabsContent?.tabs?.[ index ]?.[ 'title-medium' ] ) {
						content = content.replace(
							...
							tabsContent?.tabs?.[ index ]?.[ 'title-medium' ]
						);
						content = content.replace(
							'Give this tab a concise name',
							tabsContent?.tabs?.[ index ]?.[ 'title-medium' ]
						);
						content = content.replace(
							'tab-givethistabaconcisename',
							`tab-${ stripStringRender( tabsContent?.tabs?.[ index ]?.[ 'title-medium' ].toString() ) }`
						);
						content = content.replace(
							'tab-givethistabaconcisename',
							`tab-${ stripStringRender( tabsContent?.tabs?.[ index ]?.[ 'title-medium' ].toString() ) }`
					...
					if ( tabsContent?.tabs?.[ index ]?.[ 'title-long' ] ) {
						content = content.replace(
							...
							tabsContent?.tabs?.[ index ]?.[ 'title-long' ]
						);
					}
					// Overline.
					...
					if ( tabsContent?.tabs?.[ index ]?.[ 'list-item-1' ] ) {
						if ( ! isHTML ) {
							content = content.replace(
								`"text":"Add a single and succinct list item"`,
								`"text":"${ tabsContent?.tabs?.[ index ]?.[ 'list-item-1' ] }"`
							);
						}
						content = content.replace(
							'Add a single and succinct list item',
							tabsContent?.tabs?.[ index ]?.[ 'list-item-1' ]
						);
						content = content.replace(
							'Add a descriptive title for the column.',
							tabsContent?.tabs?.[ index ]?.[ 'list-item-1' ]
					...
					if ( tabsContent?.tabs?.[ index ]?.[ 'list-item-2' ] ) {
						if ( ! isHTML ) {
							content = content.replace(
								`"text":"Add a single and succinct list item"`,
								`"text":"${ tabsContent?.tabs?.[ index ]?.[ 'list-item-2' ] }"`
							);
						}
						content = content.replace(
							'Add a single and succinct list item',
							tabsContent?.tabs?.[ index ]?.[ 'list-item-2' ]
						);
						content = content.replace(
							'Add a descriptive title for the column.',
							tabsContent?.tabs?.[ index ]?.[ 'list-item-2' ]
					...
					if ( tabsContent?.tabs?.[ index ]?.[ 'list-item-3' ] ) {
						if ( ! isHTML ) {
							content = content.replace(
								`"text":"Add a single and succinct list item"`,
								`"text":"${ tabsContent?.tabs?.[ index ]?.[ 'list-item-3' ] }"`
							);
						}
						content = content.replace(
							'Add a single and succinct list item',
							tabsContent?.tabs?.[ index ]?.[ 'list-item-3' ]
						);
						content = content.replace(
							'Add a descriptive title for the column.',
							tabsContent?.tabs?.[ index ]?.[ 'list-item-3' ]
					...
					if ( tabsContent?.tabs?.[ index ]?.[ 'description-1' ] ) {
						content = content.replace(
							'Add context to your column. Help visitors understand the value they can get from your products and services.',
							tabsContent?.tabs?.[ index ]?.[ 'description-1' ]
						);
						content = content.replace(
						...
						content = content.replace(
							'Write a short text about your tab that will help users find what they are looking for and get access to content without further exploration.',
							tabsContent?.tabs?.[ index ]?.[ 'description-1' ]
						);
					}
					// Description 2.
					if ( tabsContent?.tabs?.[ index ]?.[ 'description-2' ] ) {
						content = content.replace(
							'Add context to your column. Help visitors understand the value they can get from your products and services.',
							tabsContent?.tabs?.[ index ]?.[ 'description-2' ]
						);

						content = content.replace(
							'This panel is hidden by default and revealed when a user clicks on the tab title. Fill it in with relevant information about its title. You can use paragraphs or bullet points to add your content here.',
							tabsContent?.tabs?.[ index ]?.[ 'description-2' ]
						);
					}
					// Description 3.
					if ( tabsContent?.tabs?.[ index ]?.[ 'description-3' ] ) {
						content = content.replace(
							'Add context to your column. Help visitors understand the value they can get from your products and services.',
							tabsContent?.tabs?.[ index ]?.[ 'description-3' ]
						);
						content = content.replace(
							'Tabs help users navigate through grouped content giving them control over what to read. Use them to connect related information and as a tool to save space.',
							tabsContent?.tabs?.[ index ]?.[ 'description-3' ]
						);
					}
				}
		...
		case 'video':
			...
			if ( baseContent?.heading?.medium ) {
				content = content.replace(
					/Add a brief headline for impact and \/ or context here/g,
					baseContent?.heading?.medium
				);
			}
			// Headline short.
			...
			if ( baseContent?.heading?.long ) {
				content = content.replace(
					/Write a compelling and inviting headline to re-hook your visitors through your content./g,
					baseContent?.heading?.long
				);
			}
			// Sentence
			if ( baseContent?.sentence?.medium ) {
				content = content.replace(
					/Use this paragraph to provide more insights writing with clear and concise language that is easy to understand. Edit and proofread your content./g,
					baseContent?.sentence?.medium
				);
			}
			// Sentence Short
		...
		case 'cards':
			...
			if ( baseContent?.sentence?.medium ) {
				content = content.replace(
					/Use a clear and attention-grabbing short paragraph to engage your audience and draw them into reading the rest of your content./g,
					baseContent?.sentence?.medium
				);
			}
			// overline
			...
			if ( columnsContent?.columns ) {
				for ( let index = 0; index < columnsContent?.columns.length; index++ ) {
					...
					if ( columnsContent?.columns?.[ index ]?.[ 'overline-short' ] ) {
						content = content.replace(
							'Overline',
							columnsContent?.columns?.[ index ]?.[ 'overline-short' ]
						);
					}
					// button.
					if ( columnsContent?.columns?.[ index ]?.[ 'button-short' ] ) {
						content = content.replace(
							'Call To Action',
							columnsContent?.columns?.[ index ]?.[ 'button-short' ]
						);
						content = content.replace(
							'Call to Action',
							columnsContent?.columns?.[ index ]?.[ 'button-short' ]
		...
		case 'testimonials':
			...
			if ( testimonialContent?.heading?.medium ) {
				content = content.replace(
					/Add a compelling title for your section to engage your audience./g,
					testimonialContent?.heading?.medium
				);
			} else if ( baseContent?.heading?.medium ) {
				content = content.replace(
					/Add a compelling title for your section to engage your audience./g,
			...
			if ( testimonialContent?.sentence?.long ) {
				content = content.replace(
					/Use this paragraph section to get your website visitors to know you. Consider writing about you or your organization, the products or services you offer, or why you exist. Keep a consistent communication style./g,
					testimonialContent?.sentence?.long
				);
		...
		case 'pricing-table':
			...
			if ( pricingTableContent?.heading?.short ) {
				content = content.replace(
					/Add a compelling title for your section to engage your audience./g,
					pricingTableContent?.heading?.short
				);
			}
			// Paragraph short
			if ( pricingTableContent?.sentence?.short ) {
				content = content.replace(
					/Add a gripping description for this featured plan/g,
					pricingTableContent?.sentence?.short
				);
			}
			// Paragraph long
			if ( pricingTableContent?.sentence?.medium ) {
				content = content.replace(
					/A pricing table assists users in selecting a suitable plan by simply and clearly differentiating product\/service features and prices. Use this as supporting text for your plans./g,
					pricingTableContent?.sentence?.medium
				);
			}
			// Paragraph medium
			if ( pricingTableContent?.sentence?.long ) {
				content = content.replace(
					/A pricing table assists users in selecting a suitable plan by simply and clearly differentiating product\/service features and prices. Use this as supporting text for your plans./g,
					pricingTableContent?.sentence?.long
				);
			}
			// overline
			...
			if ( pricingTableContent?.plans ) {
				for ( let index = 0; index < pricingTableContent?.plans.length; index++ ) {
					...
					if ( pricingTableContent?.plans?.[ index ]?.[ 'title-short' ] ) {
						content = content.replace(
							'Tab Title',
							pricingTableContent?.plans?.[ index ]?.[ 'title-short' ]
						);
					...
					if ( pricingTableContent?.plans?.[ index ]?.[ 'title-medium' ] ) {
						content = content.replace(
							'Name your plan',
							pricingTableContent?.plans?.[ index ]?.[ 'title-medium' ]
						);
					}
					// Title.
					if ( pricingTableContent?.plans?.[ index ]?.[ 'title-medium' ] ) {
					...
					if ( pricingTableContent?.plans?.[ index ]?.[ 'sentence-short' ] ) {
						content = content.replace(
							...
							pricingTableContent?.plans?.[ index ]?.[ 'sentence-short' ]
						);
					}
					// Buttons.
					if ( pricingTableContent?.plans?.[ index ]?.[ 'button-short' ] ) {
						content = content.replace(
							'Call To Action',
							pricingTableContent?.plans?.[ index ]?.[ 'button-short' ]
						);
					}
					// Feature 1.
					if ( pricingTableContent?.plans?.[ index ]?.[ 'feature-1' ] ) {
						content = content.replace(
							...
							pricingTableContent?.plans?.[ index ]?.[ 'feature-1' ]
						);
						if ( ! isHTML ) {
							content = content.replace(
								'Focus on the differences',
					...
					if ( pricingTableContent?.plans?.[ index ]?.[ 'feature-2' ] ) {
						content = content.replace(
							...
							pricingTableContent?.plans?.[ index ]?.[ 'feature-2' ]
						);
						if ( ! isHTML ) {
							content = content.replace(
								'Use a consistent language',
					...
					if ( pricingTableContent?.plans?.[ index ]?.[ 'feature-3' ] ) {
						content = content.replace(
							...
							pricingTableContent?.plans?.[ index ]?.[ 'feature-3' ]
						);
						if ( ! isHTML ) {
							content = content.replace(
								'Transmit benefits clearly',
		...
		case 'counter-or-stats':
			...
			if ( counterContent?.heading?.medium ) {
				content = content.replace(
					/Tell your story in numbers, and give your visitors useful insights./g,
					counterContent?.heading?.medium
				);
			} else if ( baseContent?.heading?.medium ) {
				content = content.replace(
					/Tell your story in numbers, and give your visitors useful insights./g,
			...
			if ( counterContent?.sentence?.medium ) {
				content = content.replace(
					/Make an impact, and share your organization's stats or achievements to interest your website visitors into learning more about you./g,
					counterContent?.sentence?.medium
				);
			} else if ( baseContent?.sentence?.medium ) {
				content = content.replace(
					/Make an impact, and share your organization's stats or achievements to interest your website visitors into learning more about you./g,
				...
				for ( let index = 0; index < counterContent?.metrics.length; index++ ) {
					...
					if ( counterContent?.metrics?.[ index ]?.[ 'title-short' ] ) {
						if ( ! isHTML ) {
							content = content.replace(
								`"title":"Stat title"`,
								`"title":"${ listContent?.list?.[ index ]?.[ 'list-item-short' ] }"`
							);
						}
						content = content.replace(
							'Stat title',
							counterContent?.metrics?.[ index ]?.[ 'title-short' ]
						);
					}
					// Price.
					if ( counterContent?.metrics?.[ index ]?.[ 'value-short' ] ) {
						if ( 0 === index ) {
							...
							content = content.replace(
								'100,110',
								counterContent?.metrics?.[ index ]?.[ 'value-short' ]
							);
						} else if ( 3 === index ) {
				...
				for ( let index = 0; index < listContent?.list.length; index++ ) {
					...
					if ( listContent?.list?.[ index ]?.[ 'list-item-long' ] ) {
						content = content.replace(
							...
							listContent?.list?.[ index ]?.[ 'list-item-long' ]
						);
						if ( ! isHTML ) {
							content = content.replace(
								'Add unique list items while keeping a consistent phrasing style and similar line lengths',
		...
		case 'list':
			...
			if ( baseContent?.heading?.medium ) {
				content = content.replace(
					/Write a clear and relevant header to keep your visitors engaged/g,
					baseContent?.heading?.medium
				);
			}
			// Paragraph medium
		...
		case 'slider':
			...
			if ( baseContent?.heading?.medium ) {
				content = content.replace(
					/Craft a captivating title to attract your audience./g,
					baseContent?.heading?.medium
				);
			}
			// Headline.
		...
		case 'post-loop':
			...
			if ( postLoopContent?.heading?.medium ) {
				content = content.replace(
					/Craft a captivating title for this section to attract your audience./g,
					postLoopContent?.heading?.medium
				);
			} else if ( baseContent?.heading?.medium ) {
				content = content.replace(
					/Craft a captivating title for this section to attract your audience./g,
			...
			if ( postLoopContent?.sentence?.short ) {
				content = content.replace(
					/Use a clear and attention-grabbing short paragraph to engage your audience and draw them into reading the rest of your content./g,
					postLoopContent?.sentence?.short
				);
			} else if ( baseContent?.sentence?.short ) {
				content = content.replace(
					/Use a clear and attention-grabbing short paragraph to engage your audience and draw them into reading the rest of your content./g,
		...
		case 'team':
			...
			if ( peopleContent?.heading?.medium ) {
				content = content.replace(
					/A short and sweet title for this section./g,
					peopleContent?.heading?.medium
				);
				content = content.replace(
					/Craft a captivating title for this section to attract your audience./g,
					peopleContent?.heading?.medium
			...
			if ( peopleContent?.sentence?.short ) {
				content = content.replace(
					/Use this space to write about your company, employee profiles and organizational culture; share your story and connect with customers./g,
					peopleContent?.sentence?.short
				);
			} else if ( baseContent?.sentence?.short ) {
				content = content.replace(
					/Use this space to write about your company, employee profiles and organizational culture; share your story and connect with customers./g,
				...
				for ( let index = 0; index < peopleContent?.people.length; index++ ) {
					...
					if ( peopleContent?.people?.[ index ]?.[ 'sentence-short' ] ) {
						content = content.replace(
							...
							peopleContent?.people?.[ index ]?.[ 'sentence-short' ]
						);
						content = content.replace(
							'Brief profile bio for this person will live here. Add an overview of this person’s role or any key information.',
							peopleContent?.people?.[ index ]?.[ 'sentence-short' ]
		...
		case 'logo-farm':
			...
			if ( baseContent?.heading?.medium ) {
				content = content.replace(
					/Tell your audience about your achievements, partners or customers./g,
					baseContent?.heading?.medium
				);
			}
			// overline
		...
		case 'location':
			...
			if ( baseContent?.sentence?.short ) {
				content = content.replace(
					/Use a brief and inviting sentence to encourage visitors to get in touch./g,
					baseContent?.sentence?.short
				);
			}
			// Headline.
		...
		case 'gallery':
			...
			if ( baseContent?.sentence?.short ) {
				content = content.replace(
					/Write with clear, concise language to inform and engage your audience. Consider what matters to them and provide valuable insights./g,
					baseContent?.sentence?.short
				);
			}
			break;
		...
		case 'featured-product':
			...
			if ( featuredContent?.heading?.medium ) {
				content = content.replace(
					/An engaging product or feature headline here/g,
					featuredContent?.heading?.medium
				);
			} else if ( baseContent?.heading?.medium ) {
				content = content.replace(
					/An engaging product or feature headline here/g,
			...
			if ( featuredContent?.sentence?.medium ) {
				content = content.replace(
					/Write a short descriptive paragraph about your product. Focus on your ideal buyer. Entice with benefits of using your product./g,
					featuredContent?.sentence?.medium
				);
			} else if ( baseContent?.sentence?.medium ) {
				content = content.replace(
					/Write a short descriptive paragraph about your product. Focus on your ideal buyer. Entice with benefits of using your product./g,
		...
		case 'form':
			...
			if ( baseContent?.sentence?.short ) {
				content = content.replace(
					/Briefly describe what the form is for or provide additional context if required. Use inviting language./g,
					baseContent?.sentence?.short
				);
			}
			// Paragraph long
			...
			if ( tocContent?.subtitles ) {
				for ( let index = 0; index < tocContent?.subtitles.length; index++ ) {
					...
					if ( tocContent?.subtitles?.[ index ]?.[ 'title-short' ] ) {
						content = content.replace(
							'Write a title for your section or related content here',
							tocContent?.subtitles?.[ index ]?.[ 'title-short' ]
						);
					}
				}
			}
