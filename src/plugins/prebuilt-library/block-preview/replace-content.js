	...
	switch (currentCategory) {
		...
		case "cards":
			...
			if ( cards_aiContent?.overline?.short ) {
				...
				content = content.replace( /Add an overline text/g, cards_aiContent?.overline?.short );
				content = content.replace( /Add an overlinE/g, cards_aiContent?.overline?.short );
				content = content.replace( /Add an overline/g, cards_aiContent?.overline?.short );
				content = content.replace( /add an overline/g,cards_aiContent?.overline?.short );
			}
			...
			// Cards 
			if ( cards_aiContent?.cards ) {
				for (let index = 0; index < cards_aiContent?.cards.length; index++) {
					// Title.
					if ( cards_aiContent?.cards?.[index]?.title?.medium ) {
						content = content.replace( "Add a Short Title Here", cards_aiContent?.cards?.[index]?.title?.medium);
					}
					if ( cards_aiContent?.cards?.[index]?.title?.short ) {
						content = content.replace( "Add a Title", cards_aiContent?.cards?.[index]?.title?.short);
					}

					// Paragraph.
					if ( cards_aiContent?.cards?.[index]?.paragraph?.medium ) {
						content = content.replace( "Use this space to add a short description. It gives enough info to earn a click.", cards_aiContent?.cards?.[index]?.paragraph?.medium)
						content = content.replace( "Use this space to add a short description. It gives enough info to earn a click.", cards_aiContent?.cards?.[index]?.paragraph?.medium);
					}
					if ( cards_aiContent?.cards?.[index]?.paragraph?.short ) {
						content = content.replace( "Add a brief description to your card.", cards_aiContent?.cards?.[index]?.paragraph?.short);
					}
					// Overline.
					if ( cards_aiContent?.cards?.[index]?.overline?.short ) {
						content = content.replace( "overline", cards_aiContent?.cards?.[index]?.overline?.short);
						content = content.replace( "Overline", cards_aiContent?.cards?.[index]?.overline?.short);
						content = content.replace( "OVERLINE", cards_aiContent?.cards?.[index]?.overline?.short);
					}
				}
