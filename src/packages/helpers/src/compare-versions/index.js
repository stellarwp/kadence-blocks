export const compareVersions = (v1, v2) => {
	...
	var v1parts = v1.split('.').map((part) => (isNaN(parseInt(part)) ? part : parseInt(part))).filter(n => !isNaN(n));
	var v2parts = v2.split('.').map((part) => (isNaN(parseInt(part)) ? part : parseInt(part))).filter(n => !isNaN(n));
