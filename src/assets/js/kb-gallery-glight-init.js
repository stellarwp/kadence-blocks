...
(function() {
	...
	var kadenceBlocksGLight = {
		...
		findGalleries: function() {
			...
			if ( foundGalleries ) {
				let carousel = [];
				for ( let item = 0; item < foundGalleries.length; item++ ) {
					carousel[item] = foundGalleries[item].querySelector( '.kt-blocks-carousel-init' );
					// if ( carousel[item] ) {
					// 	setTimeout( function() {
					// 		kadenceBlocksGLight.handleClones( carousel[item] );
					// 	}, 200 );
					// }
					let galleryClass = foundGalleries[item].classList;
					let showCaption = foundGalleries[item].getAttribute( 'data-lightbox-caption' );
					let filter = foundGalleries[item].getAttribute( 'data-image-filter' );
