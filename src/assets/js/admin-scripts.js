	...
	$('.kt-blocks-config-settings').on('click', function (e) {
		...
		if (kt_blocks_params.blockConfigSettings[selectedSlug]) {
			for (const key in kt_blocks_params.blockConfigSettings[selectedSlug]) {
				...
				const settingDefault =
					...
					typeof kt_blocks_params.blockConfig[selectedSlug][key] !== 'undefined'
						? kt_blocks_params.blockConfig[selectedSlug][key]
						: obj.default;
				switch (obj.type) {
					case 'boolean':
						settingsContent +=
							'<div class="kt-modal-settings-field"><label for="' +
							key +
							'">' +
							obj.name +
							'</label><input id="' +
							key +
							'" class="kt-block-config-input" name="' +
							key +
							'" type="checkbox" ' +
							(false === settingDefault ? '' : 'checked') +
							'></div>';
						break;
					case 'info':
					...
					case 'typoNumber':
						if (obj.units && 'em' === kt_blocks_params.blockConfig[selectedSlug][obj.units]) {
							settingsContent +=
								...
								'" class="kt-block-config-input" name="' +
								key +
								'" type="number" step="0.1" max="12" min="0.1" value="' +
								settingDefault +
								'"></div>';
						} else {
							settingsContent +=
					...
					case 'numberArray':
						settingsContent +=
							'<div class="kt-modal-settings-number-array" data-array-key="' +
							key +
							'"><h4>' +
							obj.name +
							'</h4>';
						settingsContent +=
							'<div class="kt-modal-settings-field"><label for="' +
							key +
							'[0]"><i class="dashicons dashicons-arrow-up-alt"></i></label><input id="' +
							key +
							'[0]" class="kt-block-config-input-number-array" name="' +
							key +
							'[0]" data-array-key="' +
							key +
							'" type="number" step="' +
							...
							obj.options.min +
							'" value="' +
							(settingDefault ? settingDefault[0] : '') +
							'"></div>';
						settingsContent +=
							'<div class="kt-modal-settings-field"><label for="' +
							key +
							'[1]"><i class="dashicons dashicons-arrow-right-alt"></i></label><input id="' +
							key +
							'[1]" class="kt-block-config-input-number-array" name="' +
							key +
							'[1]" data-array-key="' +
							key +
							'" type="number" step="' +
							...
							obj.options.min +
							'" value="' +
							(settingDefault ? settingDefault[1] : '') +
							'"></div>';
						settingsContent +=
							'<div class="kt-modal-settings-field"><label for="' +
							key +
							'[2]"><i class="dashicons dashicons-arrow-down-alt"></i></label><input id="' +
							key +
							'[2]" class="kt-block-config-input-number-array" name="' +
							key +
							'[2]" data-array-key="' +
							key +
							'" type="number" step="' +
							...
							obj.options.min +
							'" value="' +
							(settingDefault ? settingDefault[2] : '') +
							'"></div>';
						settingsContent +=
							'<div class="kt-modal-settings-field"><label for="' +
							key +
							'[3]"><i class="dashicons dashicons-arrow-left-alt"></i></label><input id="' +
							key +
							'[3]" class="kt-block-config-input-number-array" name="' +
							key +
							'[3]" data-array-key="' +
							key +
							'" type="number" step="' +
							...
							obj.options.min +
							'" value="' +
							(settingDefault ? settingDefault[3] : '') +
							'"></div>';
						settingsContent += '</div>';
						break;
					case 'color':
						settingsContent +=
							...
							'">' +
							obj.name +
							'</label><input class="kt-block-config-input kt-init-color" id="' +
							key +
							'" name="' +
							key +
							'" type="text" value="' +
							...
							switch (inner.type) {
								case 'number':
									settingsContent +=
										...
										'</label><input id="' +
										option +
										'" class="kt-block-config-input-array" data-array-key="' +
										key +
										'" name="' +
										option +
										'" type="number" step="' +
								...
								case 'color':
									settingsContent +=
										...
										'">' +
										inner.name +
										'</label><input class="kt-block-config-input-array kt-init-color" data-array-key="' +
										key +
										'" id="' +
										option +
										'" name="' +
								...
								default:
									settingsContent +=
										...
										'">' +
										inner.name +
										'</label><input class="kt-block-config-input-array" data-array-key="' +
										key +
										'" id="' +
										option +
										'" name="' +
					...
					default:
						settingsContent +=
							...
							'">' +
							obj.name +
							'</label><input class="kt-block-config-input" id="' +
							key +
							'" name="' +
							key +
							'" type="text" value="' +
		...
		$dialogContiner.html(
			'<p class="kt-modal-item-title">' +
				selectedName +
				' ' +
				kt_blocks_params.texts.settings +
				'</p>' +
				settingsContent
		);
		...
		$('.kt_block_button').on('click', function (event) {
			...
			function buttonStatusToggled(message) {
				if ($button.hasClass('kt-block-active')) {
					...
					setTimeout(function () {
						$button
							.addClass('kt-block-inactive')
							.removeClass('button-disabled')
							.text($button.data('inactive-label'));
					}, 2000);
				} else {
					...
					setTimeout(function () {
						$button
							.addClass('kt-block-active')
							.removeClass('button-disabled')
							.text($button.data('active-label'));
					}, 2000);
				}
			}
