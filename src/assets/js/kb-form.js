jQuery(function ($) {
	...
	var kadence_blocks_form = {
		...
		submit(e) {
			...
			if (form_data) {
				// send the request.
				form.parent('.wp-block-kadence-form')
					.find('.kadence-blocks-form-message')
					.slideUp('fast', function () {
						$(this).remove();
					});
				form.append(
					'<div class="kb-form-loading"><div class="kb-form-loading-spin"><div></div><div></div><div></div><div></div></div></div>'
				);
		...
		removeErrorNotice(form) {
			$(form).find('.kb-form-errors').remove();
		},
		markError(item, error_type) {
			let error_string = '';
			...
			if (error_type) {
				...
				if ($(item).hasClass('kb-checkbox-style')) {
					$(item)
						.parent('.kadence-blocks-form-field')
						.append(
							'<div class="kb-form-error-msg kadence-blocks-form-warning" role="alert">' +
								error_string +
								'</div>'
						);
				} else {
					$(item).after(
						'<div class="kb-form-error-msg kadence-blocks-form-warning">' + error_string + '</div>'
					);
				}
			}
		...
		addErrorNotice(form) {
			$(form).after(
				'<div class="kadence-blocks-form-message kadence-blocks-form-warning">' +
					kadence_blocks_form_params.error_message +
					'</div>'
			);
		},
		...
		validateForm(self) {
			...
			const required = self.find('[data-required="yes"]:visible');

			required.each(function (i, item) {
				// console.log( $(item).data('type') );
				const data_type = $(item).data('type');
				val = '';
