/**
 * Pure logic for the shared `ColorPicker`: turning `react-color`'s internal RGB state into the CSS
 * hex string the picker emits to its consumers, deriving the next `react-color` onChange payload
 * from an edited hex/RGB/HSL field, and toggling which of those two field rows is shown.
 *
 * Relocated from `src/style-library/helpers/color-picker.js` alongside the picker component itself
 * (see `molecules/ColorPicker.js`) — `isSameColor()`, that file's other export, stays behind in the
 * Style Library because it is used only by `ColorGradientPicker.js`, not by this picker.
 *
