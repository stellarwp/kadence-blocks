...
export function DeletePrimitiveDialog({ token, onFetchPreview, onDelete, onSuccess, onClose }) {
	...
	return (
		<Modal
			...
			className="kadence-blocks-style-library__delete-primitive-dialog"
		>
			<p className="kadence-blocks-style-library__delete-primitive-description">
			...
			{isFetching && (
				<div className="kadence-blocks-style-library__delete-primitive-loading">
					<Spinner />
					{__('Checking references…', 'kadence-blocks')}
				</div>
			)}

			{previewError && <p className="kadence-blocks-style-library__delete-primitive-error">{previewError}</p>}

			{!isFetching && preview && preview.references.length > 0 && (
				<div className="kadence-blocks-style-library__delete-primitive-refs">
					<p className="kadence-blocks-style-library__delete-primitive-refs-heading">
						{__('Tokens that reference this color:', 'kadence-blocks')}
					</p>
					<ul className="kadence-blocks-style-library__delete-primitive-refs-list">
						{preview.references.map((ref) => (
							<li key={ref.path} className="kadence-blocks-style-library__delete-primitive-ref">
								<code>{ref.path}</code>
								<span
									className={`kadence-blocks-style-library__delete-primitive-action kadence-blocks-style-library__delete-primitive-action--${ref.supported ? 'supported' : 'unsupported'}`}
								>
									{ref.supported
					...
					{hasUnsupported && (
						<p className="kadence-blocks-style-library__delete-primitive-blocked">
							{__(
			...
			<div className="kadence-blocks-style-library__delete-primitive-actions">
				<Button
					variant="primary"
					isDestructive
