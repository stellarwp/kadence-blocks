...
function OverviewCard({ title, subtitle, onClick, children }) {
	return (
		<button type="button" className="kadence-blocks-style-library__overview-card" onClick={onClick}>
			<div className="kadence-blocks-style-library__overview-card-head">
				<strong>{title}</strong>
				{subtitle ? <span>{subtitle}</span> : null}
			</div>
			<div className="kadence-blocks-style-library__overview-card-body">{children}</div>
		</button>
	);
}

/**
 * Style Library overview with at-a-glance previews per foundation group.
 *
 * @param {object}   props            Component props.
...
export function OverviewPage({ sections, tokens, values, onNavigate }) {
	...
	return (
		<div className="kadence-blocks-style-library__overview">
			...
			<div className="kadence-blocks-style-library__overview-grid">
					...
					return (
						...
						>
							{section.showColorPreview ? (
								<div className="kadence-blocks-style-library__overview-swatches">
									{colorSamples.map((color) => (
										<span
											key={color}
											className="kadence-blocks-style-library__overview-swatch"
											style={{ backgroundColor: color }}
										/>
									))}
								</div>
							) : (
								<p className="kadence-blocks-style-library__overview-hint">{section.description}</p>
							)}
						</OverviewCard>
					);
				})}
