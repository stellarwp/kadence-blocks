...
function LinkControlTransforms({ clientId }) {
	const { getBlock, blockTransforms } = useSelect(
		(select) => {
			const { getBlock: _getBlock, getBlockRootClientId, getBlockTransformItems } = select(blockEditorStore);
	...
	return (
		<div className="link-control-transform">
			<h3 className="link-control-transform__subheading">{__('Transform')}</h3>
...
export function LinkUI(props) {
	...
	return (
		<Popover placement="bottom" onClose={props.onClose} anchor={props.anchor} shift>
			<LinkControl
				hasTextControl
				hasRichPreviews
				value={link}
