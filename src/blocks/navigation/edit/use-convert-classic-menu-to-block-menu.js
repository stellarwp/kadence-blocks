...
function useConvertClassicToBlockMenu(
	...
	const convertClassicMenuToBlockMenu = useCallback(
		async ( menuId, menuName, postStatus = 'publish' ) => {
			...
			// 3. Create the `kb_navigation` Post with the blocks.
			try {
				...
				/**
				 * Immediately trigger editEntityRecord to change the kb_navigation post status to 'publish'.
				 ...
				 * If the user abandons the site editor without saving, there would still be a kb_navigation post created as draft.
				 */
