...
export default function useNavigationMenu(ref) {
	...
	const { navigationMenu, isNavigationMenuResolved, isNavigationMenuMissing } = useSelect(
		(select) => {
	...
	} = useEntityRecords('postType', `kadence_navigation`, PRELOADED_NAVIGATION_MENUS_QUERY);
...
function selectExistingMenu(select, ref) {
	...
	const isNavigationMenuPublishedOrDraft =
		editedNavigationMenu.status === 'publish' || editedNavigationMenu.status === 'draft';
