...
export class ImageEdit extends Component {
	...
	componentDidUpdate(previousProps) {
		...
		if (!previousProps.image && image) {
			const url = getUrlForSlug(image, attributes?.sizeSlug) || image.source_url;
			setAttributes({ url });
		}
	}
	...
	render() {
		...
		const getToolbarEditButton = (open) => (
			<BlockControls>
				<ToolbarGroup>
					<ToolbarButton title={__('Edit image')} icon={replace} onClick={open} />
				</ToolbarGroup>
		...
		const getInspectorControls = () => (
			<InspectorControls>
				...
				<KadencePanelBody style={styles.panelBody} panelName={'kb-image-settings-style'}>
				...
				<KadencePanelBody title={__('Link Settings')} panelName={'kb-image-link-settings'}>
				...
				>
					{canImageBeFeatured && this.getFeaturedButtonPanel(isFeaturedImage)}
					<FooterMessageControl
