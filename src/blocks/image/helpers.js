...
export const getEmbedInfoByProvider = (provider) =>
	getBlockVariations(DEFAULT_EMBED_BLOCK)?.find(({ name }) => name === provider);
...
export const findMoreSuitableBlock = (url) =>
	getBlockVariations(DEFAULT_EMBED_BLOCK)?.find(({ patterns }) => matchesPatterns(url, patterns));
...
export const createUpgradedEmbedBlock = (props, attributesFromPreview = {}) => {
	...
	const shouldCreateNewBlock =
		!isCurrentBlockWP &&
		matchedBlock &&
		(matchedBlock.attributes.providerNameSlug !== providerNameSlug || !providerNameSlug);
	...
	const wpVariation = getBlockVariations(DEFAULT_EMBED_BLOCK)?.find(({ name }) => name === 'wordpress');
