...
export default function NavigationSubmenuEdit({
		...
		(select) => {
			...
			if (selectedBlockChildren?.length === 1) {
				...
				_onlyDescendantIsEmptyLink =
					singleBlock?.name === 'core/navigation-link' && !singleBlock?.attributes?.label;
			}
	...
	const { textColor, customTextColor, backgroundColor, customBackgroundColor } = getColors(context, parentCount > 0);
	...
	const allowedBlocks =
		parentCount >= maxNestingLevel
			? ALLOWED_BLOCKS.filter((blockName) => blockName !== 'core/navigation-submenu')
			: ALLOWED_BLOCKS;
