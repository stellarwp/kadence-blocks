...
registerBlockType('kadence/form', {
	...
	keywords: [__('contact', 'kadence-blocks'), __('marketing', 'kadence-blocks'), 'KB'],
	icon: {
		src: formBlockIcon,
	},
	edit,
	save: (props) => {
			...
			if (fields[index].label && fields[index].label.includes('{privacy_policy}')) {
				...
				acceptLabel = (
					<Fragment>
						...
						<a
							href={
								undefined !== kadence_blocks_params.privacy_link &&
								'' !== kadence_blocks_params.privacy_link
									? kadence_blocks_params.privacy_link
									: '#'
							}
							target="blank"
						...
						>
							{undefined !== kadence_blocks_params.privacy_title &&
				...
				<div className={fieldClassName}>
						...
						<Fragment>
							...
							<input
								...
								data-required-message={
									fields[index].requiredMessage ? fields[index].requiredMessage : undefined
								}
								...
								aria-describedby={
									undefined !== fields[index].ariaLabel && fields[index].ariaLabel
										? `kb_field_desc_${uniqueID}_${index}`
										: undefined
								}
							/>
							<label htmlFor={`kb_field_${uniqueID}_${index}`}>
								{fields[index].label ? acceptLabel : ''}
					...
					{'accept' !== fields[index].type && (
						<Fragment>
							...
							{undefined !== fields[index].ariaLabel && fields[index].ariaLabel && (
								<span id={`kb_field_desc_${uniqueID}_${index}`} className="screen-reader-text">
									{fields[index].ariaLabel}
								</span>
							)}
							...
							{'checkbox' === fields[index].type && (
								<div
									...
									className={`kb-field kb-checkbox-style-field kb-${
										fields[index].type
									}-field kb-field-${index} kb-radio-style-${
										fields[index].inline ? 'inline' : 'normal'
									}`}
									data-required={fields[index].required ? 'yes' : undefined}
									data-required-message={
										fields[index].requiredMessage ? fields[index].requiredMessage : undefined
								...
								>
									{times(fields[index].options.length, (n) => (
										...
										>
											...
											/>
											<label htmlFor={`kb_field_${index}_${n}`}>
												{undefined !== fields[index].options[n].label
													? fields[index].options[n].label
													: ''}
											</label>
		...
		const renderFieldOutput = <Fragment>{times(fields.length, (n) => fieldOutput(n))}</Fragment>;
		...
		return (
			<div {...blockProps}>
				<form
					...
					method="post"
					data-error-message={
						messages && messages[0] && messages[0].preError ? messages[0].preError : undefined
					}
				>
					{renderFieldOutput}
					...
					<div className={submitClassName}>
						...
						<RichText.Content
							...
							className={`kb-forms-submit button kb-button-size-${
								submit[0].size ? submit[0].size : 'standard'
							} kb-button-width-${submit[0].widthType ? submit[0].widthType : 'auto'}`}
						/>
					</div>
				</form>
			</div>
	...
	deprecated: [
			...
			save: ({ attributes }) => {
				...
				const fieldOutput = (index) => {
					...
					const fieldClassName = classnames({
						'kadence-blocks-form-field': true,
						[`kb-form-field-${index}`]: index,
						[`kb-field-desk-width-${fields[index].width[0]}`]:
							fields[index].width && fields[index].width[0],
						[`kb-field-tablet-width-${fields[index].width[1]}`]:
							fields[index].width && fields[index].width[1],
						[`kb-field-mobile-width-${fields[index].width[2]}`]:
							fields[index].width && fields[index].width[2],
						[`kb-input-size-${style[0].size}`]: style[0].size,
					});
					let acceptLabel;
					let acceptLabelBefore;
							...
							{'accept' !== fields[index].type && (
								<Fragment>
									...
									{'select' === fields[index].type && (
										<select
											name={
												fields[index].multiSelect ? `kb_field_${index}[]` : `kb_field_${index}`
											}
											id={`kb_field_${uniqueID}_${index}`}
											multiple={fields[index].multiSelect ? true : false}
											data-label={fields[index].label}
										...
										>
											{undefined !== fields[index].placeholder &&
												'' !== fields[index].placeholder && (
													<option
														value=""
														disabled={true}
									...
									{'textarea' !== fields[index].type &&
										...
										'radio' !== fields[index].type && (
											<input
												...
												className={`kb-field kb-text-style-field kb-${fields[index].type}-field kb-field-${index}`}
												autoComplete={
													'' !== fields[index].auto ? fields[index].auto : undefined
												}
												data-required={fields[index].required ? 'yes' : undefined}
												data-required-message={
													fields[index].requiredMessage
		...
		},
		{
			...
			save: ({ attributes }) => {
				const { uniqueID, fields, submit, style, postID, hAlign, recaptcha, recaptchaVersion, honeyPot } =
					attributes;
				const fieldOutput = (index) => {
					const fieldClassName = classnames({
						'kadence-blocks-form-field': true,
				...
				return (
					<div className={`kadence-form-${uniqueID} kb-form-wrap${hAlign ? ' kb-form-align-' + hAlign : ''}`}>
						<form className="kb-form" action="" method="post">
							...
							<input type="hidden" name="_kb_form_post_id" value={postID} />
							<input type="hidden" name="action" value="kb_process_ajax_submit" />
							{recaptcha && <input type="hidden" name="recaptcha_response" id="kb_recaptcha_response" />}
							<input
								className="kadence-blocks-field verify"
								type="email"
								name="_kb_verify_email"
