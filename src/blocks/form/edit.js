...
class KadenceForm extends Component {
	...
	render() {
		...
		const {
			deskPaddingControl,
			tabletPaddingControl,
			mobilePaddingControl,
			borderControl,
			labelPaddingControl,
			labelMarginControl,
			submitDeskPaddingControl,
			submitTabletPaddingControl,
			submitMobilePaddingControl,
			submitBorderControl,
			messageFontBorderControl,
			messagePaddingControl,
			messageMarginControl,
			deskMarginControl,
			tabletMarginControl,
			mobileMarginControl,
			activeTab,
						...
						<Fragment>
							{fields[ index ].showLabel && (
								<label htmlFor={`kb_field_${index}`} style={{
									...
									marginBottom : ( '' !== labelFont[ 0 ].margin[ 2 ] ? labelFont[ 0 ].margin[ 2 ] + 'px' : undefined ),
									marginLeft   : ( '' !== labelFont[ 0 ].margin[ 3 ] ? labelFont[ 0 ].margin[ 3 ] + 'px' : undefined ),
								}}>{( fields[ index ].label ? fields[ index ].label :
									<span className="kb-placeholder">{'Field Label'}</span> )} {( fields[ index ].required && style[ 0 ].showRequired ?
									<span className="required" style={{ color: KadenceColorOutput( style[ 0 ].requiredColor ) }}>*</span> : '' )}</label>
							)}
							{'textarea' === fields[ index ].type && (
							...
							{actions.includes( 'email' ) && (
								...
								>
									<TextControl
										...
										help={__( 'Separate with comma for more then one email address.', 'kadence-blocks' )}
									/>
									<TextControl
