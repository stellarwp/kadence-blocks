...
export default function ThreeColumnDrag({ attributes, setAttributes, secondWidthString }) {
	const {
		padding,
		paddingUnit,
		firstColumnWidth,
	...
	if (Math.abs(secondWidthString) === parseFloat(secondWidthString)) {
		thirdWidthNumber = Math.abs(
			Math.round((parseFloat(widthNumberThirds) + parseFloat(secondWidthNumber) - 100) * 10) / 10
		);
	} else if ('left-half' === secondWidthString) {
		thirdWidthNumber = '25';
	} else if ('right-half' === secondWidthString) {
	...
	const onResizeThirds = (event, direction, elt) => {
		...
		editorDocument.getElementById('third-column-width-' + uniqueID).innerHTML =
			Math.abs(Math.round((tempSecondW + tempFirstW - 100) * 10) / 10) + '%';

		//set the temp resized styles on underlying columns
		setResizeStyles(
			`.wp-block-kadence-rowlayout.kb-row-id-${uniqueID} > .innerblocks-wrap.kb-grid-columns-3.kt-layout-inner-wrap-id${uniqueID} { grid-template-columns: minmax(0, calc( ${tempFirstW}% - (${currentGutterTotal} / 3 ) ) ) minmax(0, calc( ${tempSecondW}% - (${currentGutterTotal} / 3 ) ) )  minmax(0, calc( ${tempThird}% - (${currentGutterTotal} / 3 ) ) ) !important;}`
		);
	};
	...
	const onResizeSecond = (event, direction, elt) => {
		...
		editorDocument.getElementById('third-column-width-' + uniqueID).innerHTML =
			Math.abs(Math.round((parseFloat(tempSecondWidth) + parseFloat(tempFirstW) - 100) * 10) / 10) + '%';
		...
		<div
			...
			style={{
				paddingLeft: previewPaddingLeft
					? previewPaddingLeft + (paddingUnit ? paddingUnit : 'px')
					: fallbackPadding,
				paddingRight: previewPaddingRight
					? previewPaddingRight + (paddingUnit ? paddingUnit : 'px')
					: fallbackPadding,
				marginLeft: 'auto',
				marginRight: 'auto',
						...
						<>
							...
							<ResizableBox
								...
								size={{
									width: !secondColumnWidth
										? Math.abs(parseFloat(secondWidthNumber) + parseFloat(widthNumberThirds)) + '%'
										: Math.abs(parseFloat(secondColumnWidth) + parseFloat(firstColumnWidth)) + '%',
								}}
								minWidth={
									Math.round((!firstColumnWidth ? widthNumberThirds : firstColumnWidth) / 5) * 5 +
									10 +
									'%'
								}
								maxWidth="90%"
							...
							>
								...
								>
									...
									<span
										id={`third-right-column-width-${uniqueID}`}
										className="left-column-width-size column-width-size-handle"
									>
										{!secondColumnWidth ? secondWidthNumber + '%' : secondColumnWidth + '%'}
									</span>
									<span
										id={`third-column-width-${uniqueID}`}
										className="right-column-width-size column-width-size-handle"
									>
										{thirdWidthNumber + '%'}
									</span>
