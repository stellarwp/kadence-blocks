 ...
 * Build the row edit
 */
function RowLayoutSave({ attributes }) {
	const {
		columns,
		blockAlignment,
		inheritMaxWidth,
	...
	const renderSlider = () => {
		...
		return (
			<div
				className={`kt-blocks-carousel kt-carousel-container-dotstyle-${bgSliderSettings[0].dotStyle} kt-carousel-container-arrowstyle-${bgSliderSettings[0].arrowStyle} kb-blocks-bg-slider `}
			>
				<div
	...
	const renderVideo = () => {
		const bgVideo =
			undefined !== backgroundVideo && undefined !== backgroundVideo[0] && undefined !== backgroundVideo[0].local
				? backgroundVideo
				: [
						{
							youTube: '',
