...
export default function TwoColumnResizer({ setAttributes, attributes }) {
	const {
		padding,
		paddingUnit,
		firstColumnWidth,
	...
	const onResize = (event, direction, elt, delta) => {
		...
		setResizeStyles(
			`.wp-block-kadence-rowlayout.kb-row-id-${uniqueID} > .innerblocks-wrap.kb-grid-columns-2.kt-layout-inner-wrap-id${uniqueID} { grid-template-columns: minmax(0, calc( ${firstCol}% - (${currentGutterTotal} / 2 ) ) ) minmax(0, calc( ${secondCol}% - (${currentGutterTotal} / 2 ) ) ) !important;}`
		);
	};
					...
					{({ width }) => (
						...
						>
							...
							>
								...
								>
									{!firstColumnWidth
										? Math.abs(parseFloat(widthNumber) - 100) + '%'
										: Math.round((100 - firstColumnWidth) * 10) / 10 + '%'}
								</span>
							</span>
						</ResizableBox>
					)}
