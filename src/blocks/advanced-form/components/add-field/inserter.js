...
import PopoverInsert from './quick-inserter';
...
const defaultRenderToggle = ({
	...
	const { as: Wrapper = Button, label: labelProp, onClick, ...rest } = toggleProps;
...
class PrivateInserter extends Component {
	...
	render() {
		const { position, hasSingleBlockType, directInsertBlock, insertOnlyAllowedBlock, onSelectOrClose, inline } =
			this.props;
		...
		return (
			<Dropdown
				//className="block-editor-inserter kb-form-custom-insert"
				className={classnames({
					'block-editor-inserter': true,
					'kb-form-custom-insert': inline,
				})}
				contentClassName={classnames('block-editor-inserter__popover', { 'is-quick': true })}
				popoverProps={{ position, shift: true }}
				onToggle={this.onToggle}
				expandOnMobile
				headerTitle={__('Add a field', 'kadence-blocks')}
...
export const ComposedPrivateInserter = compose([
	withSelect((select, { clientId, rootClientId, shouldDirectInsert = true }) => {
		const {
		...
		const hasSingleBlockType =
			allowedBlocks?.length === 1 && getBlockVariations(allowedBlocks[0].name, 'inserter')?.length === 0;
		...
		return {
			insertOnlyAllowedBlock() {
				...
				function getInsertionIndex() {
					const { getBlockIndex, getBlockSelectionEnd, getBlockOrder, getBlockRootClientId } =
						select(blockEditorStore);
...
const Inserter = forwardRef((props, ref) => {
	return <ComposedPrivateInserter ref={ref} {...props} orderInitialBlockItems={undefined} />;
});
