...
function FieldBlockAppender({ rootClientId, className, onFocus, tabIndex, inline = false, getRoot = false }, ref) {
	const { parentFormBlock, insertIndex } = useSelect(
		(select) => {
			...
			const sectionBlockID =
				undefined !== sectionBlock && sectionBlock.length ? sectionBlock[sectionBlock.length - 1] : false;
			...
			if (
				undefined !== parents &&
				parents.length &&
				parents.includes(formBlockID) &&
				parents.includes(sectionBlockID)
			) {
		...
		<Inserter
			...
			renderToggle={({ onToggle, disabled, isOpen, blockTitle, hasSingleBlockType }) => {
				...
				if (hasSingleBlockType) {
					...
					label = _x('Add field', 'Generic label for block inserter button');
				}
				...
				let inserterButton = (
					<Button
						...
						label={label}
					>
						{!hasSingleBlockType && <VisuallyHidden as="span">{label}</VisuallyHidden>}
						<Icon icon={plus} />
					</Button>
				);
