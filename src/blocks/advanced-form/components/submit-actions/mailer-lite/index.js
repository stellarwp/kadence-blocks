...
export default function MailerLiteOptions({ formInnerBlocks, parentClientId, settings, save }) {
	...
	const getMailerLiteGroups = () => {
		...
		apiFetch({
			path: addQueryArgs('/kb-mailerlite/v1/get', { apikey: api, endpoint: 'groups', queryargs: ['limit=500'] }),
		})
			.then((groups) => {
				const theGroups = [];
	...
	const getMailerLiteFields = () => {
		...
		apiFetch({
			path: addQueryArgs('/kb-mailerlite/v1/get', { apikey: api, endpoint: 'fields' }),
		})
			.then((fields) => {
				const theFields = [];
				theFields.push({ value: null, label: 'None' });
					...
					{!isFetching && hasGroups && (
						<>
							...
							<div className="mailerlite-select-form-row">
								<Select
									value={
										undefined !== settings && undefined !== settings && undefined !== settings.group
											? settings.group
											: ''
									}
									onChange={(value) => {
							...
							{!settings.group && <div style={{ height: '100px' }}></div>}
							{settings.group && (
								<>
