...
export default function FieldName({ value, onChange, uniqueID }) {
	...
	return (
		<div className={`kb-form-field-name-wrap ${!hasUniqueFieldName ? 'kb-field-has-warning' : ''}`}>
			<TextControl
				...
				help={__(
					'This is the name attribute that is applied to the html input tag. Names must be unique',
					'kadence-blocks'
				)}
				value={value}
				...
				<Notice status="error" isDismissible={false}>
					...
					<ul>
						{conflictFields.map((item, index) => (
							<li key={index}>
								{__('Field Label:', 'kadence-blocks')} {item.label}
							</li>
						))}
					</ul>
				</Notice>
