 ...
 */
import { SECTION_OVERVIEW, SECTION_VARIANTS, TOKEN_TYPE_SECTIONS } from '../constants/navigation';
...
export function buildNavigationSections(tokens, variants = {}) {
	...
	const foundations = TOKEN_TYPE_SECTIONS.filter((section) => counts[section.type] > 0).map((section) => ({
		id: section.id,
		kind: 'foundation',
		label: section.label(),
	...
	if (Object.keys(variants).length > 0) {
		sections.push({
			...
			description: __('Per-block variant sets such as button styles.', 'kadence-blocks'),
